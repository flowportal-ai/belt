...
def test_load_openai_default() -> None:
    ...
    with (
        ...
        patch("flow_portal.frameworks.openai.DEFAULT_MODEL_TYPE", mock_litellm_model),
    ):
...
def test_openai_with_api_base() -> None:
    ...
    with (
        patch("flow_portal.frameworks.openai.Agent", mock_agent),
        patch(
            "flow_portal.frameworks.openai.DEFAULT_MODEL_TYPE",
            litllm_model_mock,
        ),
    ):
        AnyAgent.create(
...
def test_load_openai_agent_missing() -> None:
    with patch("flow_portal.frameworks.openai.agents_available", False):
...
def test_run_openai_with_custom_args() -> None:
    ...
    with (
        patch("flow_portal.frameworks.openai.Runner", mock_runner),
        patch("flow_portal.frameworks.openai.Agent", mock_agent),
        patch("agents.function_tool"),
        patch("flow_portal.frameworks.openai.DEFAULT_MODEL_TYPE"),
    ):
