...
```python
from flow_portal import AgentConfig, AnyAgent
# In these examples, the built-in tools will be used
from flow_portal.tools import search_web, visit_webpage
```

Check [`AgentConfig`][flow_portal.config.AgentConfig] for more info on how to configure agents.
...
Check [`AgentTrace`][flow_portal.tracing.trace.AgentTrace] for more info on the return type.

### Async

If you are running in `async` context, you should use the equivalent [`create_async`][flow_portal.AnyAgent.create_async] and [`run_async`][flow_portal.AnyAgent.run_async] methods:
